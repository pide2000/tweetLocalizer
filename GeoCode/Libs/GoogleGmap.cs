...
namespace Lucuma.Libs
{
    ...
    public class GoogleGmapConfig : GeoProviderConfig, IGeoProviderConfig
    {
        ...
        {
            UrlPattern = "http://maps.googleapis.com/maps/api/geocode/json?sensor=false"; // default 
                ...
                if (!String.IsNullOrEmpty(json))
                {
                    ...
                    if (result["status"].GetStringValue() == GoogleResultStatus.OK.ToString() && result["results"].Count()>0)
                    {
                        ...
                        foreach (var loc in result["results"])
                        {
                            ...
                            foreach (var adrComp in loc["address_components"]) {
                                if (adrComp["types"][0].ToString().Equals("country")) {
                                    Location.Country= adrComp["short_name"].GetStringValue();
                                }
